...
struct ComptimeScope {
    ...
    const std::unordered_map<std::string,Value>& getLocalValues() const {return localValues;}
...
private:
    ...
    void codegenAssert(llvm::Value* condition, Position location, llvm::StringRef message = "Assertion failed");
    ...
    Value evalBinaryOp(Token::Kind op,Value& lhs,Value &rhs,Type lhsType,Position location = Position());
    ...
    llvm::Function* getFunctionProto(const FunctionDecl& decl,llvm::StringMap<Value> compile_time = llvm::StringMap<Value>());
...
private:
    ...
    llvm::StringMap<const FunctionDecl&> functions = llvm::StringMap<const FunctionDecl&>();
