use super::CFG;
use super::{Value,IrType,Inst,Function,Opcode,InstructionData,Block,BlockData,Entity};
...
impl<'a> FunctionBuilder<'a> {
    ...
    pub fn stack_alloc(&mut self,size: usize) -> Value {
        let value = self.cfg().make_inst(InstructionData::AllocaSize(Opcode::Alloca,size));
        self.func.value_types.insert(value,IrType::Ptr(box IrType::UInt(8)));
        value
    }
    pub fn stack_alloc_ty(&mut self,ty: IrType) -> Value {
        let value = self.cfg().make_inst(InstructionData::Alloca(Opcode::Alloca,box ty));
        self.func.value_types.insert(value,IrType::Ptr(box IrType::UInt(8)));
        value
    }

    pub fn iconst(&mut self,type_: IrType,imm: impl Into<i64>) -> Value {
        let value = self.cfg().make_inst(InstructionData::UnaryImm(Opcode::Iconst,imm.into()));
        self.func.value_types.insert(value, type_);
        value
    }

    pub fn iadd(&mut self,lhs: Value,rhs: Value) -> Value {
        let value = self.cfg().make_inst(InstructionData::IntBinary(Opcode::Iadd,lhs,rhs));
    ...
    pub fn isub(&mut self,lhs: Value,rhs: Value) -> Value {
        let value = self.cfg().make_inst(InstructionData::IntBinary(Opcode::Isub,lhs,rhs));
        let ty = self.get_value_type(&lhs).clone();
        self.func.value_types.insert(value,ty);
        value
    }
    ...
    pub fn store(&mut self,val: Value,elem: Value) {
        self.cfg().make_inst(InstructionData::Store(Opcode::Store,val,elem));
    }

    pub fn jump(&mut self,block: Block) {
        self.cfg().make_inst(InstructionData::Branch(Opcode::Br,block,None));
    }

    pub fn jump_if(&mut self,block: Block,val: Value) {
        self.cfg().make_inst(InstructionData::Branch(Opcode::BrNz,block,Some(val)));
    }

    pub fn jump_ifnot(&mut self,block: Block,val: Value) {
        self.cfg().make_inst(InstructionData::Branch(Opcode::BrZ,block,Some(val)));
    }
    ...
    pub fn return_(&mut self,val: Value) {
        self.cfg().make_inst(InstructionData::Return(Opcode::Return,val));
    }
