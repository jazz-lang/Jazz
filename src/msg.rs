...
impl Msg {
  pub fn message(&self) -> String {
    match *self {
      ...
      UnknownField(ref field, ref ty) => format!("unknown field `{}` for type `{}`", field, ty),
      ...
      IfCondType(ref ty) => format!("`if` expects condition of type `bool` but got `{}`.", ty),
      ...
      ThrowsDifference(_) => "use of `throws` in method `{}`needs to match super class".into(),
      MethodNotOverridable(ref name) => {
        format!("method `{}` in super class not overridable.", name)
      }
      TypesIncompatible(ref na, ref nb) => format!("types `{}` and `{}` incompatible.", na, nb),
      ...
      ExpectedClassElement(ref token) => format!("field or method expected but got {}.", token),
