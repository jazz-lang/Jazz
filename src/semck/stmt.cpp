...
void Typechecker::typecheckReturnStmt(ReturnStmt& stmt) {
    ...
    stmt.getReturnValue()->setMoved(true,stmt.getReturnValue()->getLocation());
    movedValues.push_back(stmt.getReturnValue());
}

void Typechecker::typecheckVarStmt(VarStmt& stmt) {
    typecheckVar(stmt.getDecl(), false);
}
...
void Typechecker::typecheckIfStmt(IfStmt& ifStmt) {
    ...
    for (auto& stmt : ifStmt.getThenBody()) {
        typecheckStmt(stmt);
        for (auto& stmt_: ifStmt.getElseBody()) {
            if (stmt_->isIfStmt() || stmt_->isCompoundStmt()) {
                scopeEnd();
            }
        }
    }
...
void Typechecker::typecheckStmt(std::unique_ptr<Stmt>& stmt) {
    switch (stmt->getKind()) {
        case StmtKind::ComptimeStmt:  {
            ...
            typecheckStmt(comptimeStmt.stmt);
            break;
        }
        ...
        case StmtKind::ForStmt: {
            ...
            if (!llvm::cast<ForStmt>(*stmt).shouldInline()) {
                ...
                auto *variable = forStmt.getVariable();
                typecheckExpr(forStmt.getRangeExpr());
                variable->setType(forStmt.getRangeExpr().getSubExprs()[1]->getType());
                currentControlStmts.push_back(&forStmt);
                getCurrentModule()->addToSymbolTable(*variable,false);
