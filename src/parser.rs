...
impl<'a> Parser<'a> {
    ...
    fn create_binary(&mut self,tok: Token,left: Box<Expr>,right: Box<Expr>) -> Box<Expr> {
        let op = match tok.kind {
            TokenKind::Eq => {
                return expr!(ExprKind::Assign(left,right),tok.position)
            }
            TokenKind::Or => "||",
            TokenKind::And => "&&",
            TokenKind::BitOr => "|",
    ...
    fn parse_lambda(&mut self) -> EResult {
        ...
        let params = if tok.kind == TokenKind::Or {
            ...
            self.parse_comma_list(TokenKind::BitOr, |f| {
                f.parse_function_param()
            })?
        };
    ...
    fn lit_int(&mut self) -> EResult {
        ...
        if let TokenKind::LitInt(i,_,_) = tok.kind {
            Ok(expr!(
            ExprKind::ConstInt(i.parse().unwrap()),
            pos
        ))
        } else {
            unreachable!()
    ...
    fn lit_float(&mut self) -> EResult {
        ...
        if let TokenKind::LitFloat(c) = tok.kind {
            Ok(expr!(
            ExprKind::ConstFloat(c.parse().unwrap()),
            pos
        ))
        } else {
            unreachable!()
