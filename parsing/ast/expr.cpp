...
llvm::APSInt Expr::getConstInt() const {
    switch (getKind()) {
        case ExprKind::VarExpr: {
            auto* decl = llvm::cast<VarExpr>(this)->getDecl();
            ...
            if (auto* varDecl = llvm::dyn_cast<VarDecl>(decl)) {
                if (!varDecl->getType().isMutable() && varDecl->getInitializer()) {
                    return varDecl->getInitializer()->getConstInt();
                }
            }
        ...
        case ExprKind::UnaryExpr:
            return llvm::cast<UnaryExpr>(this)->getConstInt();

        case ExprKind::BinaryExpr:
            return llvm::cast<BinaryExpr>(this)->getConstInt();
...
bool Expr::isLvalue() const {
    switch (getKind()) {
        case ExprKind::ComptimeExpr:  {
            auto* val = llvm::cast<ComptimeExpr>(this);

            return val->expr.get()->isLvalue();
        }
        case ExprKind::VarExpr:
        case ExprKind::StringLiteralExpr:
        case ExprKind::CharacterLiteralExpr:
...
void Expr::setMoved(bool moved,Position location) {
    if (auto* varExpr = llvm::dyn_cast<VarExpr>(this)) {
        switch (varExpr->getDecl()->getKind()) {
            case DeclKind::ParamDecl:
                llvm::cast<ParamDecl>(varExpr->getDecl())->setMoved(moved,location);
                break;
            case DeclKind::VarDecl:
                llvm::cast<VarDecl>(varExpr->getDecl())->setMoved(moved,location);
                break;
            default:
                break;
        }
...
std::unique_ptr<Expr> Expr::instantiate(const llvm::StringMap<Type>& genericArgs) const {
    ...
    switch (getKind()) {
        ...
        case ExprKind::ComptimeExpr: {
            auto* comptimeExpr = llvm::cast<ComptimeExpr>(this);

            instantiation = comptimeExpr->expr->instantiate(genericArgs);
            break;
        }
        case ExprKind::StringLiteralExpr: {
...
std::vector<const Expr*> Expr::getSubExprs() const {
    ...
    switch (getKind()) {
        ...
        case ExprKind::ComptimeExpr: {
            auto* comptimeExpr = llvm::cast<ComptimeExpr>(this);
            subExprs.push_back(&comptimeExpr->getExpr());
            break;
        }
        case ExprKind::TypenameExpr:
        case ExprKind::SizeofExpr:
...
std::string CallExpr::getRefinedFunctionName() const {
    return ::getRefinedFunctionName(::getReceiverType(*this), getFunctionName(), {});
}
...
llvm::APSInt UnaryExpr::getConstInt() const {
    auto operand = getOperand().getConstInt();
...
bool jazz::isBuiltinOp(Token::Kind op, Type left, Type right) {
...
llvm::APSInt BinaryExpr::getConstInt() const {
    auto lhs = getLHS().getConstInt();
    auto rhs = getRHS().getConstInt();
