...
FunctionDecl* FunctionTemplate::instantiate(const llvm::StringMap<Type>& genericArgs) {
    jazz_assert(!genericParams.empty() && !genericArgs.empty());
...
std::string jazz::getRefinedFunctionName(Type receiver, llvm::StringRef name, llvm::ArrayRef<Type> genericArgs,llvm::ArrayRef<ParamDecl> params) {
    std::string result;
    ...
    if (params.size() > 0) {
        for (auto& param : params) {
            result += param.getType().getRefinedTypeName();
        }
    }
    return result;
}

std::string FunctionDecl::getRefinedName() const {
    Type receiver = getTypeDecl() ? getTypeDecl()->getType() : Type();
    return getRefinedFunctionName(receiver, getName(), getGenericArgs());
}
...
MethodDecl::MethodDecl(DeclKind kind, FunctionProto proto, TypeDecl& typeDecl, std::vector<Type>&& genericArgs, AccessLevel accessLevel, Position location,bool override_)
: FunctionDecl(kind, std::move(proto), std::move(genericArgs), accessLevel, *typeDecl.getModule(), location), typeDecl(&typeDecl),
  mutating(false),override_(override_) {}
...
std::vector<ParamDecl> jazz::instantiateParams(llvm::ArrayRef<ParamDecl> params, const llvm::StringMap<Type>& genericArgs) {
    return map(params,
               [&](const ParamDecl& param) { return ParamDecl(param.getType().resolve(genericArgs), param.getName(), param.getLocation()); });
}

std::string TypeDecl::getRefinedName() const {
    return getRefinedTypeName(getName(), getGenericArgs());
}
...
bool TypeDecl::isCopy() const {
    return llvm::any_of(getInterfaces(), [&](Type type) { return type.getName() == "Copy"; });
}
...
TypeDecl* TypeTemplate::instantiate(llvm::ArrayRef<Type> genericArgs) {
    jazz_assert(genericArgs.size() == genericParams.size());
...
std::string FieldDecl::getRefinedName() const {
    return (getParent()->getRefinedName() + "." + getName()).str();
}
...
std::unique_ptr<Decl> Decl::instantiate(const llvm::StringMap<Type>& genericArgs, llvm::ArrayRef<Type> genericArgsArray) const {
    switch (getKind()) {
        ...
        case DeclKind::LinkDecl: 
            llvm_unreachable("LinkDecl cannot be generic or declared inside some type");
        case DeclKind::VarDecl: {
