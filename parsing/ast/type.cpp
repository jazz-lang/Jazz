...
#define DEFINE_BUILTIN_TYPE_GET_AND_IS(TYPE, NAME) \
    Type Type::get##TYPE(bool isMutable, Position location) { \
...
bool Type::isImplicitlyCopy() const {
    switch (getKind()) {
        case TypeKind::BasicType:
            return !getDecl() || getDecl()->passByValue() || getName() == "SharedPtr";
        case TypeKind::ArrayType:
            return false;
        case TypeKind::TupleType:
            return llvm::all_of(llvm::cast<TupleType>(typeBase)->getElements(),
                                [&](auto& element) { return element.type.isImplicitlyCopy(); });
        case TypeKind::FunctionType:
            return true;
        case TypeKind::PointerType:
            return true;
        case TypeKind::OptionalType:
            return getWrappedType().isImplicitlyCopy();
    }
...
Type Type::makePointer() const {
    return PointerType::get(*this,isMutable(),this->getLocation());
}

template<typename T>
static Type getType(T&& typeBase, bool isMutable, Position location) {
...
Type BasicType::get(llvm::StringRef name, llvm::ArrayRef<Type> genericArgs, bool isMutable, Position location) {
    return getType(BasicType(name, genericArgs), isMutable, location);
}

Type ArrayType::get(Type elementType, int64_t size, bool isMutable, Position location) {
    return getType(ArrayType(elementType, size), isMutable, location);
}

Type TupleType::get(std::vector<TupleElement>&& elements, bool isMutable, Position location) {
    return getType(TupleType(std::move(elements)), isMutable, location);
}

Type FunctionType::get(Type returnType, std::vector<Type>&& paramTypes, bool isMutable, Position location) {
    return getType(FunctionType(returnType, std::move(paramTypes)), isMutable, location);
}

Type PointerType::get(Type pointeeType, bool isMutable, Position location) {
    return getType(PointerType(pointeeType), isMutable, location);
}

Type OptionalType::get(Type wrappedType, bool isMutable, Position location) {
    return getType(OptionalType(wrappedType), isMutable, location);
}

bool jazz::operator==(const TupleElement& a, const TupleElement& b) {
    return a.name == b.name && a.type == b.type;
}

void jazz::appendGenericArgs(std::string& typeName, llvm::ArrayRef<Type> genericArgs) {
...
std::string jazz::getRefinedTypeName(llvm::StringRef typeName, llvm::ArrayRef<Type> genericArgs) {
    std::string result = typeName;
    appendGenericArgs(result, genericArgs);
    return result;
}

std::vector<ParamDecl> FunctionType::getParamDecls(Position location) const {
...
llvm::StringRef Type::getName() const {
    if (isFunctionType()) {
        ...
        auto arrayType = llvm::cast<ArrayType>(typeBase);
        auto string = std::string();

        string = string + "[";
        string = string + arrayType->getElementType().getName().str();
        string = string + ";";
        string = string + std::to_string(arrayType->getSize());
        string = string + "]"; 
    }
    return llvm::cast<BasicType>(typeBase)->getName();
}

std::string Type::getRefinedTypeName() const {
    return llvm::cast<BasicType>(typeBase)->getRefinedName();
}
...
bool jazz::operator==(Type lhs, Type rhs) {
...
std::ostream& jazz::operator<<(std::ostream& stream, Type type) {
    type.printTo(stream, true);
    return stream;
}

llvm::raw_ostream& jazz::operator<<(llvm::raw_ostream& stream, Type type) {
