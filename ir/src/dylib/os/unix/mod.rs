use crate::dylib::util::{ensure_compatible_types, cstr_cow_from_bytes};
...
impl Library {
    ...
    #[inline]
    pub fn new<P: AsRef<OsStr>>(filename: P) -> crate::dylib::Result<Library> {
    ...
    pub fn open<P>(filename: Option<P>, flags: raw::c_int) -> crate::dylib::Result<Library>
    where P: AsRef<OsStr> {
        let filename = match filename {
            None => None,
            Some(ref f) => Some(unwrap_err!(cstr_cow_from_bytes(f.as_ref().as_bytes()))),
        };
        with_dlerror(move || {
            let result = unsafe {
    ...
    pub unsafe fn get<T>(&self, symbol: &[u8]) -> crate::dylib::Result<Symbol<T>> {
        ensure_compatible_types::<T, *mut raw::c_void>();
        let symbol = unwrap_err!(cstr_cow_from_bytes(symbol));
